...
concurrency:
  ...
  # Backlog workflow_dispatch/schedule runs with no explicit issue_number must
  # share a stable key so concurrent backlog launches cannot pick the same task.
  ...
  group: >-
    gh-aw-${{ github.workflow }}-${{
      ...
      ((github.event_name == 'workflow_dispatch' || github.event_name == 'schedule') && 'backlog') ||
      github.run_id
    }}
...
3. For each implementable issue (check memory — skip if already attempted):
   ...
   k. **Trigger the reviewer**:
      - If the repository is running with `PIPELINE_MVP_MODE=true`, do **not** dispatch `pr-review-agent` after PR creation. MVP mode relies on async post-merge review audit instead of a pre-merge review pass.
